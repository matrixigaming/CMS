...
<?php
//echo "<pre>"; print_r($data['betWinDataArr']); echo "</pre>";die;
?>
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active">Report - Game wise Jackpot RTP</li>
          </ol>
        </section>
@if(Session::has('message')) <div class="alert alert-info"> {{Session::get('message')}} </div> @endif
        <!-- Main content -->
        <section class="content">
{!! Form::open(array('url' => '/reports/jackpot_rtp', 'id'=>'qwerty')) !!}
                <div class="row">
            ...
            <div class="col-md-12">
              ...
              <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                  <li class="active"><a href="#manageUsers" data-toggle="tab">Game wise Jackpot RTP</a></li>
                </ul>
                  ...
                  <table class="table table-bordered table-striped fullDetailTable">
                    ...
                    <tbody>
                        @if(!empty($data['betWinDataArr'])) 
                            @php $total_jackpot_bet_amount = 0; $total_jackpot_win_amount = 0;@endphp
                            @foreach($data['betWinDataArr'] as $val) 
                                @php 
                                    $jackpot_bet_amount = isset($val->jackpot_bet_amount)? $val->jackpot_bet_amount : 0;
                                    $jackpot_win_amount= isset($val->jackpot_win_amount)? $val->jackpot_win_amount : 0;                        
                                    $total_jackpot_bet_amount +=$jackpot_bet_amount; $total_jackpot_win_amount +=$jackpot_win_amount;
                                @endphp
                          <tr>
                              <td>
                            @if($val->game_icon)
                            ...
                            @endphp
                            <img class="img-responsive" src="{{ url($filename) }}" alt="game icon - {{ $val->game_name }}">
                        @else
                            <img class="img-responsive" src="{{ url('uploads/no-image_th.jpg') }}" alt="game icon - {{ $val->game_name }}"  >
                        @endif</td>
                            <td>{{ $val->game_name }} </td>
                            <td>{{ $jackpot_bet_amount }}</td>
                            <td>{{ $jackpot_win_amount }}</td>
                            <td>{{ $jackpot_bet_amount > 0 ? number_format((($jackpot_win_amount)*100)/$jackpot_bet_amount, 2) : 0 }}%</td>                        
                          </tr>
                          @endforeach 
                          <tfoot>
                            <td></td>
                            <th>Total</th>
                            <th>{{number_format($total_jackpot_bet_amount,2)}}</th>
                            <th>{{number_format($total_jackpot_win_amount,2)}}</th>
                            <th>{{ $total_jackpot_bet_amount>0?number_format((($total_jackpot_win_amount)*100)/$total_jackpot_bet_amount, 2):0 }}%</th>
                          </tfoot>
                      @else
                        <tr>
